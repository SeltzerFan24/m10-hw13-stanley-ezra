...
async function getWeather(query) {
  ...
  const response = (await fetch(
    ...
    if (response.cod === "404") throw new Error('location not found')
      // create weather icon URL
      var iconUrl = 'https://openweathermap.org/img/wn/' +
        response.weather[0].icon +
        '@2x.png'
      var description = response.weather[0].description
      var actualTemp = response.main.temp
      ...
      var updatedAt = new Date(response.dt * 1000)
      // this object is used by displayWeatherInfo to update the HTML
      return {
        coords: response.coord.lat + ',' + response.coord.lon,
